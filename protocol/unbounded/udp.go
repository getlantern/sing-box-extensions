...
func (h *UDPOverQUICHandler) receiveLoop() {
	for {
		...
		default:
			...
			h.logger.Debug("receiveLoop() receiving ", len(data), " bytes")
...
func (h *UDPOverQUICHandler) handleUDPPacket() {
	for {
		...
		h.logger.Debug("handleUDPPacket() received re-assembled ", len(data), " bytes")
		...
		if h.router == nil { // Client side (no router means we're the unbounded client)
			h.logger.Debug("Client reading ", sourceAddr, " -> ", destAddr, " of bytes: ", len(payload))
			h.routeToClientConn(sourceAddr, destAddr, payload)
			continue
		}
		h.logger.Debug("Server reading ", sourceAddr, " -> ", destAddr, " of bytes: ", len(payload))
...
func (h *UDPOverQUICHandler) cleanupSession(sessionKey string) {
	h.logger.Debug("Session ", sessionKey, " deleted")
	h.sessionsMux.Lock()
...
func (c *UDPSessionConn) ReadFrom(p []byte) (n int, addr net.Addr, err error) {
	...
	case packet := <-c.readCh:
		...
		c.handler.logger.Trace("ServerConn ReadFrom ", packet.addr.String(), " data:", string(p))
...
func (c *UDPSessionConn) WriteTo(p []byte, addr net.Addr) (n int, err error) {
	...
	case c.writeCh <- packet:
		c.handler.logger.Trace("ServerConn WriteTo ", addr.String(), " data:", string(p))
		return len(p), nil
...
func (c *UDPSessionConn) ReadPacket(buffer *buf.Buffer) (M.Socksaddr, error) {
	...
	case packet := <-c.readCh:
		...
		c.handler.logger.Trace("ServerConn ReadPacket ", packet.addr.String(), " bytes:", len(packet.data))
...
func (c *UDPSessionConn) WritePacket(buffer *buf.Buffer, addr M.Socksaddr) error {
	...
	case c.writeCh <- packet:
		c.handler.logger.Trace("ServerConn WritePacket ", addr.String(), " bytes:", len(packet.data))
		return nil
	case <-c.getWriteDeadlineChannel():
...
func (c *UDPSessionConn) sendPacketOverQUIC(packet *UDPPacket) error {
	...
	c.handler.logger.Debug("Server sending data of ", len(packets), " bytes to ", c.session.sourceAddr.String())
...
func (c *ClientUDPConn) ReadFrom(p []byte) (n int, addr net.Addr, err error) {
	select {
	case <-c.closed:
		c.handler.logger.Trace("ClientConn ReadFrom: closed")
		return 0, nil, net.ErrClosed
	case packet := <-c.readCh:
		n = copy(p, packet.data)
		c.handler.logger.Trace("ClientConn ReadFrom ", packet.addr.String())
		return n, packet.addr, nil
	case <-c.getReadDeadlineChannel():
		c.handler.logger.Trace("ClientConn ReadFrom: read deadline exceeded")
...
func (c *ClientUDPConn) WriteTo(p []byte, addr net.Addr) (n int, err error) {
	...
	c.handler.logger.Trace("ClientConn WriteTo ", addr.String())
	...
	for _, chunk := range chunks {
		c.handler.logger.Debug("Client sending data of ", len(chunk), " bytes to ", c.destination.String())
		if err := c.handler.quicConn.SendDatagram(chunk); err != nil {
			return 0, err
		}
	}
...
func (c *ClientUDPConn) Close() error {
	c.closeOnce.Do(func() {
		close(c.closed)
		c.handler.logger.Debug("ClientConn closed")
